    ...
    <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 overflow-hidden shadow-sm">
      ...
      <div className="divide-y divide-stone-100 dark:divide-stone-800">
        {steps.map((step) => (
          ...
          >
            {step.completed ? (
              <LuCircleCheckBig className="w-5 h-5 text-rose-500 shrink-0 mt-0.5" />
            ) : (
              <LuCircle className="w-5 h-5 text-stone-300 dark:text-stone-600 shrink-0 mt-0.5" />
            )}
            ...
            {!step.completed && (
              <LuArrowRight className="w-4 h-4 text-stone-400 shrink-0 mt-0.5" />
            )}
          </button>
        ))}
      </div>
